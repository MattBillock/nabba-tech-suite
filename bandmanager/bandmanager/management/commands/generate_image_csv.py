...
from api.models import Contest, Director, PerformanceSlot, Venue, Partner
...
class Command(BaseCommand):
    ...
    def build_performance_schedule(self, contest):
        ...
        """
        performance_slots_for_contest = PerformanceSlot.objects.filter(contest=contest).order_by('performance_time').all()
        ...
        for slot in performance_slots_for_contest:
            ...
            schedule_dict[schedule_day].append({
                ...
                "performance_times": {
                    "start_timestamp": slot.performance_time.strftime("%Y-%m-%dT%H:%M:%S-05:00"),
                    "end_timestamp": slot.performance_end_time.strftime("%Y-%m-%dT%H:%M:%S-05:00")
                }
            })
    ...
    def build_partner_details(self, contest):
        ...
        """
        partners = Partner.objects.filter(contest=contest).all()
        result = []
        for partner in partners:
            result.append({
                "image_src": partner.image_src,
                "name": partner.name,
                "link": partner.link,
    ...
    def handle(self, *args, **options):
        ...
        print(contest)
        bands = contest.bands.order_by('name').all()
        print(bands)
